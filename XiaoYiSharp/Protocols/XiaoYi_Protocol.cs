...
namespace XiaoYiSharp.Protocols
{
    public class XiaoYi_Protocol
    {
        ...
        // 唤醒词检测
        public static string Listen_Detect(string text,string url="")
        {
            string message = @"{
                    ""type"": ""listen"",
                    ""state"": ""detect"",
            ...
            message = message.Replace("<图片>", url);
            //Console.WriteLine($"发送的消息: {message}");
            return message;
        }
        ...
        // 3. 中止消息
        public static string Abort(string sessionId = "")
        {
            string message = @"{
                ""session_id"": ""<会话ID>"",
                ""type"": ""abort"",
        ...
        // 设备描述
        public static string Device_Info(string descriptors)
        {
            string message = @"{
                ""session_id"": ""<会话ID>"",
                ""type"": ""iot"",
            ...
            message = message.Replace("<设备描述JSON>", descriptors);
            return message;
        }

        // 设备状态
        public static string Device_Status(string status)
        {
            string message = @"{
                ""session_id"": ""<会话ID>"",
                ""type"": ""iot"",
            ...
            message = message.Replace("\n", "").Replace("\r", "").Replace("\r\n", "").Replace(" ", "");
            message = message.Replace("<会话ID>", "");
            message = message.Replace("<状态JSON>", status);
            return message;
        }
